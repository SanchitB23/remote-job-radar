...
export default function JobListingComponent({
  ...
  className,
  onJobClick,
}: JobListingComponentProps): JSX.Element {
      ...
      <AnimatePresence>
        {activeItem ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="pointer-events-none absolute inset-0 z-10 bg-black/10 backdrop-blur-xl"
          />
        ) : null}
      </AnimatePresence>
      <AnimatePresence>
      ...
      <div className={`relative flex items-start p-6 ${className || ""}`}>
        <div className="relative flex w-full flex-col items-center gap-4 px-2">
          {jobs.map((role) => (
            ...
            >
              <motion.div layoutId={`workItemLogo-${role.company}`}>{role.logo}</motion.div>
